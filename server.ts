...
let cachedAesJs = "";
let cachedTestCookie = "";

async function getChallengeCookie(html: string): Promise<string> {
  try {
    const origin = new URL(REMOTE_WAMP_API_URL).origin;
    if (!cachedAesJs) {
      console.log(`[Proxy] Fetching aes.js from ${origin}/aes.js...`);
      const aesRes = await fetch(`${origin}/aes.js`);
      cachedAesJs = await aesRes.text();
    }
    const scriptRegex = /<script>([\s\S]*?)<\/script>/;
    const match = html.match(scriptRegex);
    if (!match) throw new Error("No challenge script found in HTML");
    ...
    vm.runInContext(cachedAesJs, sandbox);
    vm.runInContext(match[1], sandbox);
    
    const cookieMatch = sandbox.document.cookie.match(/__test=([^;]+)/);
    if (!cookieMatch) throw new Error("No __test cookie generated by script");
    return cookieMatch[1];
  } catch (err) {
    console.error("[Proxy] Failed to solve challenge:", err);
    throw err;
  }
}
...
if (USE_REMOTE_WAMP_API) {
  app.all("/wamp-api/*", express.raw({ type: "*/*", limit: "5mb" }), async (req, res) => {
    try {
      ...
      const sendRequest = async (cookieValue: string) => {
      ...
      let response = await sendRequest(cachedTestCookie);
      let text = "";
      let solvedNewCookie = false;

      const contentType = response.headers.get("content-type") || "";
      if (contentType.includes("text/html")) {
        text = await response.text();
        if (text.includes("slowAES")) {
          console.log("[Proxy] JS challenge detected. Solving...");
          const newCookie = await getChallengeCookie(text);
          cachedTestCookie = newCookie;
          solvedNewCookie = true;
          console.log("[Proxy] Challenge solved. Retrying request with cookie.");
          response = await sendRequest(cachedTestCookie);
        }
      }
      ...
      if (text && !solvedNewCookie) {
        // If we read the text initially but it wasn't the challenge, return it as-is
